...
if [ ! -f "$LIBS_FILE" ]; then
    echo "Error: $LIBS_FILE not found." >&2
    exit 1
fi
...
echo "[DEBUG] Detecting library changes since $PREV_TAG" >&2
echo "[DEBUG] Previous versions: A=$prev_A, B=$prev_B" >&2
...
if [ "$prev_A" = "$prev_B" ]; then
    ...
    echo "Library change detected." >&2
    echo "patch"
fi